...
void print_1D(vector<double> &X){
    ...
    for(int j=0;j<X.size();j++){
        cout<<(long long)(X[j]*10000000000)<<" ";
        // cout<<X[j]<<" ";
    }
    cout<<"\n--------------------\n";
}
    ...
    */
    // for(int op=1;op<=Onum;op++){
        ...
        */
        ...
        coordinate_data[1].resize(100); //numbers of nodes
        coordinate_data[2].resize(100);
...
void Xfunc(int op){
    ...
    */
    ...
    for(int i=1;i<weight_data.size();i++){
        ...
        weight_data[i]=read_filter_2D(fin,coordinate_data[i].size(),coordinate_data[i-1].size()+1);
        for(int j=0;j<coordinate_data[i].size();j++){
            while(weight_data[i][j].size()!=coordinate_data[i-1].size()+1)weight_data[i][j].push_back(0.1*(rand()%5+1));//자동으로 채워주는 건데, 체크는 나중에 해 보자고~!
        }
    }
